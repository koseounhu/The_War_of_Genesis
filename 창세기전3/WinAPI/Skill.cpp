		...
		if (IMAGEMANAGER->findImage("ªÏ∂ÛµÚ_Ω∫≈≥")->getFrameX() == IMAGEMANAGER->findImage("ªÏ∂ÛµÚ_Ω∫≈≥")->getMaxFrameX())
		{
			...
			if (_enemyXY[0].frame > IMAGEMANAGER->findImage("Ω∫≈≥¿Ã∆Â∆Æ1")->getMaxFrameX())
			{
				if (_tick % 3 == 0)
				{
					...
					if (_temp[39].frame > IMAGEMANAGER->findImage("Ω∫≈≥¿Ã∆Â∆Æ3")->getMaxFrameX())
					for (int i = 40; i < 80; i++)
					{
						_temp[i].frame++;
			...
			for (int i = 0; i < _temp.size(); i++)
			{
				...
				if (_temp[i].start && i > 39 && _temp[i].frame< IMAGEMANAGER->findImage("Ω∫≈≥¿Ã∆Â∆Æ3")->getMaxFrameX())
				{
...
void Skill::DownSkill(Player* _pl)
{
	...
	if (_skill[5] == 1)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			{
				IMAGEMANAGER->findImage("Ω∫≈≥¿Ã∆Â∆Æ1")->alphaFrameRender(getMemDC(), _enemyXY[i].x - 80, _enemyXY[i].y - 50, 120, _enemyXY[i].frame, 0);
				IMAGEMANAGER->findImage("Ω∫≈≥¿Ã∆Â∆Æ1±§ø¯")->alphaFrameRender(getMemDC(), _enemyXY[i].x - 80, _enemyXY[i].y - 50, 180, _enemyXY[i].frame, 0);
			}
		}
